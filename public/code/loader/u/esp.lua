return function()
    ...
    local function setVisible(o, val)
        ...
        pcall(function() o.Visible = val end)
    end

    local function setEnabled(o, val)
        if not o then return end
        pcall(function() o.Enabled = val end)
    end

    local function withinViewport(x, y)
        if not cam or not cam.ViewportSize then return false end
    ...
    local function getColor(h, m)
        local p = h / m
        if p > 0.7 then return Color3.fromRGB(0,255,0)
    ...
    rs.RenderStepped:Connect(function()
        ...
        for p,b in pairs(objs.boxes) do
            ...
            if hum then
                if esp.showhealth and validOn2 then setText(ht,("%d/%d"):format(hum.Health,hum.MaxHealth),Vector2.new(hd.X,hd.Y+5),getColor(hum.Health,hum.MaxHealth)) else setVisible(ht,false) end
                ...
                else setVisible(hb,false) end
            end

            if esp.showchams then pcall(function() chm.Adornee=ch chm.FillColor=col chm.Enabled=true end) else setEnabled(chm,false) end
    ...
    function esp:disable()
        ...
        for name,t in pairs(objs) do
            for key,o in pairs(t) do
                if name=="skeletons" then removeSkeleton(key) else setVisible(o,false) setEnabled(o,false) end
            end
        end
    end
