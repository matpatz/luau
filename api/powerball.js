...
export default async function handler(req,res){
    ...
    if(action==="getStats" || (!guess && !action)){
        return json({balance:user.balance||0,wins:user.wins||0,wrong:user.wrong||0,cooldown:user.cooldown||0,jackpot:state.jackpot||BASE_JACKPOT})
    }
    ...
    if(guess===state.code){
        ...
        return json({valid:true,payout,balance:user.balance,wins:user.wins,wrong:user.wrong,cooldown:user.cooldown})
    }
    ...
    return json({valid:false,jackpot:newJackpot,balance:user.balance,wins:user.wins,wrong:user.wrong,cooldown:user.cooldown})
}
