...
local tween = get(game:GetService("TweenService"))
local tweeninfo = TweenInfo.new
local input = get(game:GetService("UserInputService"))
local run = get(game:GetService("RunService"))
local core = get(game:GetService("CoreGui"))
--if gethui then local hui = gethui() end

local Utility = {}
local Objects = {}function Kavo:DraggingEnabled(frame, parent)
    ...
    input.InputChanged:Connect(function(input)
        if input == dragInput and dragging then
            ...
            local frameW, frameH = parent.AbsoluteSize.X, parent.AbsoluteSize.Y
            newX = math.clamp(newX, 0, screenW - frameW)
            newY = math.clamp(newY, 0, screenH - frameH)
...
function Kavo.CreateLib(kavName, themeList)
	...
	Main.Position = UDim2.new(0, screenSize.X * 0.1, 0, screenSize.Y * 0.1) -- 10% offset from top-left
